...
import { ApoptosisProposalResp, deserializeApoptosisProposalResp, serializeApoptosisProposalResp, } from '../../../src/types/ApoptosisProposalResp';
...
export const ApopotosizeInternalApiTest = async (from: ShardusTypes.Node, to: ShardusTypes.Node): Promise<void> => {
    try {
        ...
        const resp = await Comms.askBinary<ApoptosisProposalResp, ApoptosisProposalResp>(to, 'apoptosize', obj, serializeApoptosisProposalResp, deserializeApoptosisProposalResp, { sender_id: to.id }, '0x546567890ab');
    ...
    try {
        ...
        const resp = await Comms.askBinary<ApoptosisProposalReq, ApoptosisProposalResp>(to, 'apoptosize', obj, serializeApoptosisProposalReq, deserializeApoptosisProposalResp, { sender_id: from.id }, '0x1234567890ab');
        const endTime = Date.now();
    ...
    try {
        ...
        const resp = await Comms.askBinary<ApoptosisProposalReq, ApoptosisProposalResp>(to, 'apoptosize', obj, serializeApoptosisProposalReq, deserializeApoptosisProposalResp, { sender_id: from.id }, '0x2234567890ab');
        const endTime = Date.now();
    ...
    try {
        ...
        await Comms.tellBinary<ApoptosisProposalReq>([to], 'apoptosize', obj, serializeApoptosisProposalReq, {
            sender_id: from.id,
            tracker_id: '0x11234567890ab',
        }, false, '0x11234567890ab');
        const endTime = Date.now();
