...
jest.mock('../../../../src/network/debugMiddleware', () => ({
  ...
  isDebugModeMiddlewareLow: jest.fn(),
}))
 ...
 * - Mocking the registerEndpoints method to replace the infinite loop with a single iteration
 * - Verifying that the loop body executes correctly (stringifyReduce and crypto.hash calls)
 ...
 *
 * A better design would use a controlled loop with a maximum duration or an asynchronous
 * approach with setInterval that could be more easily tested.
 */
...
describe('NestedCounters', () => {
  ...
  describe('printArrayReport', () => {
    ...
    test('should handle null or undefined subArray', () => {
  ...
  describe('registerEndpoints', () => {
    ...
    test('counts endpoint should return JSON when accept header is application/json', () => {
      ...
      // Find the counts endpoint handler
      const registerCalls = (Context.network.registerExternalGet as jest.Mock).mock.calls
      const countsEndpointCall = registerCalls.find((call) => call[0] === 'counts')
      const countsHandler = countsEndpointCall[2]

      // Add some data
      nestedCounters.countEvent('category1', 'subcategory1')

      // Call the handler with a request that has JSON accept header
      const mockReq = { headers: { accept: 'application/json' } }
      countsHandler(mockReq, mockResponse)
    ...
    test('counts-reset endpoint should reset eventCounters', () => {
      ...
      // Find the counts-reset endpoint handler
      const registerCalls = (Context.network.registerExternalGet as jest.Mock).mock.calls
      const resetEndpointCall = registerCalls.find((call) => call[0] === 'counts-reset')
      const resetHandler = resetEndpointCall[2]
    ...
    test('rare-counts-reset endpoint should reset rareEventCounters', () => {
      ...
      // Find the rare-counts-reset endpoint handler
      const registerCalls = (Context.network.registerExternalGet as jest.Mock).mock.calls
      const resetEndpointCall = registerCalls.find((call) => call[0] === 'rare-counts-reset')
      const resetHandler = resetEndpointCall[2]
    ...
    test('debug-inf-loop endpoint should set infLoopDebug to true and execute the loop', () => {
      // Set up crypto mock with type assertion to bypass type checking
      nestedCounters.crypto = { hash: mockCryptoHash } as any

      // Replace the while loop with a mock implementation
      const originalRegisterEndpoints = nestedCounters.registerEndpoints
      nestedCounters.registerEndpoints = function () {
        Context.network.registerExternalGet('debug-inf-loop', isDebugModeMiddleware, (req, res) => {
          ...
          // Instead of an infinite loop, just run the body once
          const s = 'asdf'
      ...
      // Find the debug-inf-loop endpoint handler
      const registerCalls = (Context.network.registerExternalGet as jest.Mock).mock.calls
      const infLoopEndpointCall = registerCalls.find((call) => call[0] === 'debug-inf-loop')
      const infLoopHandler = infLoopEndpointCall[2]

      // Call the handler
      infLoopHandler({}, mockResponse)

      // Check response
      expect(mockResponse.write).toHaveBeenCalledWith('starting inf loop, goodbye')
      expect(mockResponse.end).toHaveBeenCalled()

      // Check utils.stringifyReduce was called
      expect(utils.stringifyReduce).toHaveBeenCalledTimes(2)

      // Check crypto.hash was called
      expect(mockCryptoHash).toHaveBeenCalledTimes(1)
      expect(mockCryptoHash).toHaveBeenCalledWith('mocked-string')

      // Restore original method
      nestedCounters.registerEndpoints = originalRegisterEndpoints
    })

    test('debug-inf-loop endpoint should handle null crypto', () => {
      // Ensure crypto is null
      nestedCounters.crypto = null
    ...
    test('debug-inf-loop-off endpoint should set infLoopDebug to false', () => {
      ...
      // Find the debug-inf-loop-off endpoint handler
      const registerCalls = (Context.network.registerExternalGet as jest.Mock).mock.calls
      const infLoopOffEndpointCall = registerCalls.find((call) => call[0] === 'debug-inf-loop-off')
      const infLoopOffHandler = infLoopOffEndpointCall[2]

      // Set infLoopDebug to true
      nestedCounters.infLoopDebug = true

      // Call the handler
      infLoopOffHandler({}, mockResponse)
