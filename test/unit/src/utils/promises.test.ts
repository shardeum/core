...
test('groupResolvePromises() > should exit early due to total wins', async () => {
    ...
    const res = await groupResolvePromises([generatePromise('resolved', 10), generatePromise('rejected', 100)], evaluationFunction, 1, 1);
...
test('groupResolvePromises() > should exit early due to total losses', async () => {
    ...
    const res = await groupResolvePromises([generatePromise('resolved', 100), generatePromise('rejected', 10)], evaluationFunction, 1, 1);
