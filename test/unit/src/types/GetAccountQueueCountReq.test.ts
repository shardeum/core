...
import { cGetAccountQueueCountReqVersion, deserializeGetAccountQueueCountReq, serializeGetAccountQueueCountReq, } from '../../../../src/types/GetAccountQueueCountReq';
...
describe('serializeGetAccountQueueCountReq', () => {
    test('should serialize with root = true', () => {
        ...
        expectedStream.writeUInt16(TypeIdentifierEnum.cGetAccountQueueCountReq);
...
describe('deserializeGetAccountQueueCountReq Serialization and Deserialization Together', () => {
    test.each([{ accountIds: [] }, { accountIds: ['accountId1', 'accountdId2'] }])('should serialize and deserialize maintaining data integrity for %#', (originalObj) => {
        const stream = new VectorBufferStream(0);
