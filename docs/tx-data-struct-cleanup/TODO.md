...
|-----| ----------- | ----------- |
|Signature Verification| Shardus calls dapp fn `app.validateTransaction()` to verify tx signature before adding to tx queue | Shardus now calls `app.validateTransaction()` during transaction intake |
