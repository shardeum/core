...
class Statistics extends EventEmitter {
    constructor(baseDir: string, config, { counters = [], watchers = {}, timers = [], manualStats = [], fifoStats = [], ringOverrides = {}, fifoOverrides = {}, }: {
        counters: string[];
        watchers: any;
        timers: any;
        manualStats: string[];
    ...
    setFifoStat(fifoStatName, value: number) {
        ...
        if (!fifoStat)
            throw new Error(`fifoStat '${fifoStatName}' is undefined.`);
        fifoStat.save(value);
    }
    ...
    _initializeManualStats(counterDefs = [], ringOverrides = {}) {
        ...
        for (const name of counterDefs) {
            ...
            manualStats[name] = new ManualRing(count);
        }
        return manualStats;
    }
    _initializeFifoStats(fifoStatsDefs = [], statsOverrides = {}) {
        ...
        for (const name of fifoStatsDefs) {
            ...
            fifoStats[name] = new FifoStats(count);
        }
        return fifoStats;
    }
    _takeSnapshot() {
