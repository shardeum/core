...
export function updateStateHashesByCycleMap(counter: Cycle['counter'], stateHash: P2P.SnapshotTypes.StateHashes, stateHashesByCycle: Iterable<readonly [
    number,
...
export function updateReceiptHashesByCycleMap(counter: Cycle['counter'], receiptHash: P2P.SnapshotTypes.ReceiptHashes, receiptHashesByCycle: Iterable<readonly [
    number,
...
export function updateSummaryHashesByCycleMap(counter: Cycle['counter'], summaryHashes: P2P.SnapshotTypes.SummaryHashes, summaryHashesByCycle: Iterable<readonly [
    number,
...
export async function savePartitionAndNetworkHashes(shard: CycleShardData, partitionHashes: hashMap, networkHash: P2P.SnapshotTypes.NetworkStateHash): Promise<void> {
...
export async function saveReceiptAndNetworkHashes(shard: CycleShardData, receiptMapHashes: hashMap, networkReceiptHash: P2P.SnapshotTypes.NetworkReceiptHash): Promise<void> {
...
export async function saveSummaryAndNetworkHashes(shard: CycleShardData, summaryHashes: hashMap, summaryReceiptHash: P2P.SnapshotTypes.NetworkSummarytHash): Promise<void> {
...
export async function readOldNetworkHash(): Promise<{
    hash: string;
}> {
    try {
...
export async function readOldPartitionHashes(): Promise<{
    hash: string;
    partitionId: string;
}[]> {
    try {
...
export async function calculateOldDataMap(shardGlobals: StateManager.shardFunctionTypes.ShardGlobals, nodeShardDataMap: StateManager.shardFunctionTypes.NodeShardDataMap, oldPartitionHashMap: Map<number, string>, lastSnapshotCycle: number): Promise<Map<P2P.SnapshotTypes.PartitionNum, ShardusTypes.AccountsCopy[]>> {
    ...
    ShardFunctions.computeNodePartitionDataMap(shardGlobals, nodeShardDataMap, nodes, partitionShardDataMap, nodes, true, false);
    for (const [partitionId, partitonObj] of partitionShardDataMap) {
        try {
            ...
            const oldAccountCopiesInPartition = await Context.storage.getOldAccountCopiesByCycleAndRange(lastSnapshotCycle, lowAddress, highAddress);
...
export function copyOldDataToDataToMigrate(oldDataMap: Map<number, ShardusTypes.AccountsCopy[]>, dataToMigrate: Map<number, ShardusTypes.AccountsCopy[]>): void {
...
export function getMissingPartitions(shardGlobals: StateManager.shardFunctionTypes.ShardGlobals, oldDataMap: Map<number, ShardusTypes.AccountsCopy[]>): number[] {
...
export function registerDownloadRoutes(network: NetworkClass, oldDataMap: Map<number, ShardusTypes.AccountsCopy[]>, oldPartitionHashMap: Map<number, string>): void {
    let dataToSend = {};
