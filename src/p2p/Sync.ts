...
import { addStandbyJoinRequests, debugDumpJoinRequestList, getLastHashedStandbyList, getStandbyNodesInfoMap, } from './Join/v2';
...
const routes = {
    external: [newestCycleRoute, cyclesRoute],
};
...
export async function sync(activeNodes: P2P.SyncTypes.ActiveNode[]) {
    ...
    do {
        ...
        if (prevCycles.length < 1) {
            ...
            error(`sync-getting-cycles failed to get ${start} - ${end}  expectedMissing:${expectedMissing} missingListCount:${missingListCount}` + Utils.safeStringify(missingList));
            if (config.p2p.hackForceCycleSyncComplete === true && missingListCount === 0) {
                error(`hack-fix: go active missing count is 0`);
                return true;
            }
        ...
        if (prepended < 1)
            throw new Error('Unable to prepend any previous cycles');
...
type SyncNode = Partial<Pick<P2P.SyncTypes.ActiveNode, 'ip' | 'port'> & Pick<P2P.NodeListTypes.Node, 'externalIp' | 'externalPort'>>;
export async function syncNewCycles(activeNodes: SyncNode[]) {
    ...
    while (CycleChain.newest.counter < newestCycle.counter) {
        ...
        const { cycles: nextCycles, responders } = await getCycles(activeNodes, CycleChain.newest.counter + 1);
...
export function digestCycle(cycle: P2P.CycleCreatorTypes.CycleRecord, source: string) {
    ...
    if (config.p2p.useSyncProtocolV2 || config.p2p.writeSyncProtocolV2) {
        ...
        if (source === 'syncV2' && newNodeListHash !== cycle.nodeListHash)
        ...
        if (source === 'syncV2' && newArchiverListHash !== cycle.archiverListHash)
            warn(`sync:digestCycle source: ${source} cycle: ${cycle.counter} patching archiverlisthash ${cycle.archiverListHash} -> ${newArchiverListHash}`);
    ...
    if (logFlags.important_as_error)
        info(`digestCycle ${Utils.safeStringify(cycle)} from ${source}... note: CycleChain.newest.counter: ${Utils.safeStringify(CycleChain.newest)} CycleCreator.currentCycle: ${CycleCreator.currentCycle}`);
    ...
    if (logFlags.p2pNonFatal) {
        ...
        debugDumpJoinRequestList(Array.from(getStandbyNodesInfoMap().values()), `sync.digestCycle: standby-map ${cycle.counter}`);
    }
...
function applyNodeListChange(change: P2P.CycleParserTypes.Change, raiseEvents: boolean, cycle: P2P.CycleCreatorTypes.CycleRecord | null) {
    NodeList.addNodes(change.added.map((joined) => NodeList.createNode(joined)), 'applyNodeListChange');
    NodeList.updateNodes(change.updated, raiseEvents, cycle);
    if (change.removed[0] !== 'all')
...
export async function getNewestCycle(activeNodes: SyncNode[]): Promise<P2P.CycleCreatorTypes.CycleRecord> {
    if (activeNodes == null)
        throw new Error('null activeNodes');
    if (activeNodes.length < 1)
        throw new Error('empty activeNodes');
    ...
    const eqFn = (item1, item2) => {
        try {
            if (item1.newestCycle.counter === item2.newestCycle.counter)
                return true;
            return false;
        }
        catch (err) {
    ...
    if (activeNodes.length > 10)
        redundancy = 3;
    const { topResult: response, winningNodes: _responders } = await robustQuery(activeNodes, queryFn, eqFn, redundancy, true);
    if (!response?.newestCycle)
        throw new Error('warning: getNewestCycle: no newestCycle yet');
    const newestCycle = response.newestCycle as P2P.CycleCreatorTypes.CycleRecord;
    return newestCycle;
}
async function getCycles(activeNodes: SyncNode[], start: number, end?: number): Promise<{
    cycles: P2P.CycleCreatorTypes.CycleRecord[];
    responders: Partial<Pick<P2P.SyncTypes.ActiveNode, "ip" | "port"> & Pick<P2P.NodeListTypes.Node, "externalIp" | "externalPort">>[];
}> {
    if (start < 0)
        start = 0;
    if (end !== undefined) {
    ...
    if (activeNodes.length > 10)
        redundancy = 3;
    const { topResult: response, winningNodes: _responders } = await robustQuery(activeNodes, queryFn, util.isDeepStrictEqual, redundancy, true);
    const cycles = response as P2P.CycleCreatorTypes.CycleRecord[];
    const valid = validateCycles(cycles);
    if (valid)
        return { cycles, responders: _responders };
}
