...
import { isDebugModeMiddleware, isDebugModeMiddlewareLow, isDebugModeMiddlewareMultiSig, } from '../network/debugMiddleware';
...
class MemoryReporting {
    ...
    gatherStateManagerReport(): void {
        if (this.shardus && this.shardus.stateManager) {
            ...
            this.addToReport('Patcher', 'history', Utils.safeStringify(this.shardus.stateManager.accountPatcher.syncFailHistory), 1);
            this.addToReport('Patcher', 'insync', `${inSync}`, 1);
        }
    }
    getCPUTimes(): object[] {
