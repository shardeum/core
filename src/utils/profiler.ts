...
class Profiler {
    ...
    profileSectionStart(sectionName: string, internal = false): void {
        if (!Context.config?.debug?.enableBasicProfiling)
            return;
    ...
    scopedProfileSectionStart(sectionName: string, internal = false, messageSize: number = cNoSizeTrack): void {
        if (Context.config?.debug?.enableScopedProfiling === false)
            return;
        ...
        if (messageSize != cNoSizeTrack && messageSize != cUninitializedSize) {
            ...
            stat.c += 1;
            if (messageSize > stat.max)
                stat.max = messageSize;
            if (messageSize < stat.min)
                stat.min = messageSize;
    ...
    getTotalBusyInternal(): {
        duty: number;
        netInternlDuty: number;
        netExternlDuty: number;
    } {
    ...
    printAndClearReport(): string {
        ...
        const d1 = Context.config?.debug?.highResolutionProfiling ? this.cleanInt(1e6) : this.cleanInt(1);
            ...
            if (Object.prototype.hasOwnProperty.call(this.scopedSectionTimes, key)) {
                ...
                if (section.resp.c > 0) {
                    const dataReport = {
                        ...
                        c: section.resp.c,
                        avg: humanFileSize(Math.ceil(section.resp.total / section.resp.c)),
                    };
                ...
                if (numberStat.c > 0) {
                    const dataReport = {
                        ...
                        c: numberStat.c,
                        avg: humanFileSize(Math.ceil(numberStat.total / numberStat.c)),
                    };
