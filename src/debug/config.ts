...
export function isDebugModeAnd(predicate: (config: DebugConfigurations | Partial<DebugConfigurations>) => boolean): boolean {
...
export function getDevPublicKeyMaxLevel(clearance?: DevSecurityLevel): string | null {
    ...
    for (const key in devPublicKeys) {
        if (devPublicKeys[key]) {
            if (clearance && devPublicKeys[key] >= clearance)
                return key;
            else {
