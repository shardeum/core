...
export function serializeGetAccountQueueCountResp(stream: VectorBufferStream, obj: GetAccountQueueCountResp, root = false): void {
    if (root) {
    ...
    if (obj === false) {
        ...
        obj.committingAppData.forEach((data) => stream.writeBuffer(stateManager.app.binarySerializeObject(AppObjEnum.AppData, data)));
        stream.writeUInt16(obj.accounts.length);
        obj.accounts.forEach((account) => stream.writeBuffer(stateManager.app.binarySerializeObject(AppObjEnum.AppData, account)));
    }
    if (root) {
        stream.writeUInt16(TypeIdentifierEnum.cGetAccountQueueCountResp);
    }
