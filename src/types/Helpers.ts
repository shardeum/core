...
export const responseSerializer = <T>(data: T, serializerFunc: (stream: VectorBufferStream, obj: T, root?: boolean) => void): VectorBufferStream => {
...
export const responseDeserializer = <T>(data: VectorBufferStream, deserializerFunc: (stream: VectorBufferStream, root?: boolean) => T): T => {
    data.position = 0;
...
export const requestSerializer = <T>(data: T, serializerFunc: (stream: VectorBufferStream, obj: T, root?: boolean) => void): VectorBufferStream => {
    const bufferInitialSize = estimateBinarySizeOfObject(data);
    const serializedPayload = new VectorBufferStream(bufferInitialSize);
    serializerFunc(serializedPayload, data, true);
...
export const getStreamWithTypeCheck = (payload: Buffer, typeId: number, customErrorLog?: string): VectorBufferStream => {
...
export const requestErrorHandler = (apiRoute: InternalRouteEnum, errorType: RequestErrorEnum, header: AppHeader, opts?: {
