...
import { deserializeWrappedDataFromQueue, serializeWrappedDataFromQueue, WrappedDataFromQueueSerializable, } from './WrappedDataFromQueue';
...
export function serializeGetAccountDataWithQueueHintsResp(stream: VectorBufferStream, obj: GetAccountDataWithQueueHintsRespSerializable, root = false): void {
    if (root) {
...
export function deserializeGetAccountDataWithQueueHintsResp(stream: VectorBufferStream): GetAccountDataWithQueueHintsRespSerializable {
    ...
    if (accountDataPresent === 1) {
        ...
        for (let i = 0; i < length; i++) {
            accountData.push(deserializeWrappedDataFromQueue(stream));
        }
    }
