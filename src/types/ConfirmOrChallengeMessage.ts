...
export function serializeConfirmOrChallengeMessage(stream: VectorBufferStream, obj: ConfirmOrChallengeMessageSerializable, root = false): void {
    if (root) {
...
export function deserializeConfirmOrChallengeMessage(stream: VectorBufferStream): ConfirmOrChallengeMessageSerializable {
    const version = stream.readUInt8();
    if (version > cConfirmOrChallengeMessageVersion) {
        throw new Error(`ConfirmOrChallengeMessageDeserializer expected version ${cConfirmOrChallengeMessageVersion}, got ${version}`);
    }
