...
export function serializeRequestStateForTxPostResp(stream: VectorBufferStream, resp: RequestStateForTxPostResp, root = false): void {
    if (root) {
    ...
    resp.stateList.forEach((item) => serializeWrappedDataResponse(stream, item));
}
